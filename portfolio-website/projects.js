...
function displayProjects() {
    ...
    projects.forEach((project) => {
        ...
        projectCard.className = "bg-stone-200 rounded-lg p-6 hover:bg-red-100 transition"
        projectCard.innerHTML = `
            ...
            <div class="flex flex-wrap gap-2 mb-4">
                ${project.tags.map((tag) => `<span class="bg-red-600 text-white text-xs px-2 py-1 rounded">${tag}</span>`).join("")}
            </div>
            <a href="${project.link}" class="text-red-700 hover:text-amber-800">View Project →</a>
        `
